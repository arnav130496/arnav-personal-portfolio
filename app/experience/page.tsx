      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative border-l border-gray-800 ml-4">
          {EXPERIENCE.map((job, index) => (
            <div key={index} className="mb-12 last:mb-0 pl-8 relative group">
              {/* Timeline Dot */}
              <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-black border-2 border-blue-500 transition-transform group-hover:scale-125" />

              <div className="flex flex-col gap-1 mb-3">
                <span className="text-xs font-mono text-blue-500 tracking-widest uppercase">
                  {job.period}
                </span>
                <h2 className="text-2xl font-bold text-gray-100">{job.role}</h2>
                <h3 className="text-lg text-gray-400 font-medium">{job.company}</h3>
              </div>

              {/* Technology Tokens - Always Visible */}
              ...
              <button 
                ...
                className="text-xs font-bold text-gray-500 hover:text-white transition-colors flex items-center gap-1 mb-2"
              >
              ...
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${
                ...
                <ul className="mt-4 space-y-3 bg-gray-900/30 p-4 rounded-lg border border-gray-800">
                  {job.highlights.map((highlight, i) => (
                    <li key={i} className="flex items-start text-gray-300 text-sm">
                      <span className="mr-3 mt-2 h-1 w-1 rounded-full bg-blue-500 shrink-0" />
                      {highlight}
                    </li>
                  ))}
                </ul>
